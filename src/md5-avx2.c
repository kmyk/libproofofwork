...
bool pow_md5_mine(uint8_t *mask, uint8_t *target, uint8_t *buffer, uint64_t *size) {
    ...
    static const uint32_t x14 = message_bytes * 8;
    ...
    uint32_t c1 = md5_c0;
    uint32_t d1 = md5_d0;
    md5_round1(&a1,b1,c1,d1, x0 ,  7, 0xd76aa478, md5_f1);
    md5_round1(&d1,a1,b1,c1, x1 , 12, 0xe8c7b756, md5_f1);
    md5_round1(&c1,d1,a1,b1, x2 , 17, 0x242070db, md5_f1);
    md5_round1(&b1,c1,d1,a1, x3 , 22, 0xc1bdceee, md5_f1);
    md5_round1(&a1,b1,c1,d1, x4 ,  7, 0xf57c0faf, md5_f1);
    md5_round1(&d1,a1,b1,c1, x5 , 12, 0x4787c62a, md5_f1);
    md5_round1(&c1,d1,a1,b1, x6 , 17, 0xa8304613, md5_f1);
    md5_round1(&b1,c1,d1,a1, x7 , 22, 0xfd469501, md5_f1);
    md5_round1(&a1,b1,c1,d1, x8 ,  7, 0x698098d8, md5_f1);
    md5_round1(&d1,a1,b1,c1, x9 , 12, 0x8b44f7af, md5_f1);
    md5_round1(&c1,d1,a1,b1, x10, 17, 0xffff5bb1, md5_f1);
    uint64_t cnt = 0;
...
#pragma omp parallel for shared(found) reduction(+:cnt)
    ...
    repeat_ascii (i8 ) { if (found) break;
        const uint32_t x11 = i11 | ((uint32_t)i10 << 8) | ((uint32_t)i9 << 16) | ((uint32_t)i8 << 24);
    ...
    repeat_ascii (i2 ) {
#undef repeat_ascii
        __m256i y13 = _mm256_set1_epi32('#' | ((uint32_t)i2 << 8) | ((uint32_t)i1 << 16) | (0x80 << 24)) + _mm256_set_epi32(7,6,5,4,3,2,1,0);
    for (uint8_t i3 = '#'; i3+7 < 'z'+1; i3 += 8, y13 += _mm256_set1_epi32(8)) {
        ...
        md5_round (&b,c,d,a, x11, 22, 0x895cd7be, md5_f);
        md5_round (&a,b,c,d, x12,  7, 0x6b901122, md5_f);
        ...
        md5_round (&a,b,c,d, x1 ,  5, 0xf61e2562, md5_g);
        md5_round (&d,a,b,c, x6 ,  9, 0xc040b340, md5_g);
        md5_round (&c,d,a,b, x11, 14, 0x265e5a51, md5_g);
        md5_round (&b,c,d,a, x0 , 20, 0xe9b6c7aa, md5_g);
        md5_round (&a,b,c,d, x5 ,  5, 0xd62f105d, md5_g);
        md5_round (&d,a,b,c, x10,  9, 0x02441453, md5_g);
        md5_round (&c,d,a,b, x15, 14, 0xd8a1e681, md5_g);
        md5_round (&b,c,d,a, x4 , 20, 0xe7d3fbc8, md5_g);
        md5_round (&a,b,c,d, x9 ,  5, 0x21e1cde6, md5_g);
        md5_round (&d,a,b,c, x14,  9, 0xc33707d6, md5_g);
        md5_round (&c,d,a,b, x3 , 14, 0xf4d50d87, md5_g);
        md5_round (&b,c,d,a, x8 , 20, 0x455a14ed, md5_g);
        md5_roundv(&a,b,c,d, y13,  5, 0xa9e3e905, md5_g);
        md5_round (&d,a,b,c, x2 ,  9, 0xfcefa3f8, md5_g);
        md5_round (&c,d,a,b, x7 , 14, 0x676f02d9, md5_g);
        md5_round (&b,c,d,a, x12, 20, 0x8d2a4c8a, md5_g);
        md5_round (&a,b,c,d, x5 ,  4, 0xfffa3942, md5_h);
        md5_round (&d,a,b,c, x8 , 11, 0x8771f681, md5_h);
        md5_round (&c,d,a,b, x11, 16, 0x6d9d6122, md5_h);
        md5_round (&b,c,d,a, x14, 23, 0xfde5380c, md5_h);
        md5_round (&a,b,c,d, x1 ,  4, 0xa4beea44, md5_h);
        md5_round (&d,a,b,c, x4 , 11, 0x4bdecfa9, md5_h);
        md5_round (&c,d,a,b, x7 , 16, 0xf6bb4b60, md5_h);
        md5_round (&b,c,d,a, x10, 23, 0xbebfbc70, md5_h);
        md5_roundv(&a,b,c,d, y13,  4, 0x289b7ec6, md5_h);
        md5_round (&d,a,b,c, x0 , 11, 0xeaa127fa, md5_h);
        md5_round (&c,d,a,b, x3 , 16, 0xd4ef3085, md5_h);
        md5_round (&b,c,d,a, x6 , 23, 0x04881d05, md5_h);
        md5_round (&a,b,c,d, x9 ,  4, 0xd9d4d039, md5_h);
        md5_round (&d,a,b,c, x12, 11, 0xe6db99e5, md5_h);
        md5_round (&c,d,a,b, x15, 16, 0x1fa27cf8, md5_h);
        md5_round (&b,c,d,a, x2 , 23, 0xc4ac5665, md5_h);
        md5_round (&a,b,c,d, x0 ,  6, 0xf4292244, md5_i);
        md5_round (&d,a,b,c, x7 , 10, 0x432aff97, md5_i);
        md5_round (&c,d,a,b, x14, 15, 0xab9423a7, md5_i);
        md5_round (&b,c,d,a, x5 , 21, 0xfc93a039, md5_i);
        md5_round (&a,b,c,d, x12,  6, 0x655b59c3, md5_i);
        md5_round (&d,a,b,c, x3 , 10, 0x8f0ccc92, md5_i);
        md5_round (&c,d,a,b, x10, 15, 0xffeff47d, md5_i);
        md5_round (&b,c,d,a, x1 , 21, 0x85845dd1, md5_i);
        md5_round (&a,b,c,d, x8 ,  6, 0x6fa87e4f, md5_i);
        md5_round (&d,a,b,c, x15, 10, 0xfe2ce6e0, md5_i);
        md5_round (&c,d,a,b, x6 , 15, 0xa3014314, md5_i);
        md5_roundv(&b,c,d,a, y13, 21, 0x4e0811a1, md5_i);
        md5_round (&a,b,c,d, x4 ,  6, 0xf7537e82, md5_i);
        ...
        if (unlikely(not _mm256_testz_si256(cmp_a, cmp_a))) {
            md5_round (&d,a,b,c, x11, 10, 0xbd3af235, md5_i);
            md5_round (&c,d,a,b, x2 , 15, 0x2ad7d2bb, md5_i);
            md5_round (&b,c,d,a, x9 , 21, 0xeb86d391, md5_i);
